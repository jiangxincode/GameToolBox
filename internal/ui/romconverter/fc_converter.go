...
// NewFCConverterView creates the FC/NES ROM converter UI.
func NewFCConverterView(w fyne.Window) fyne.CanvasObject {
...
// Format selection
formats := romconverter.GetFCFormats()
...
sourceFormatSelect.Selected = string(romconverter.FormatNES) // Default

targetFormatSelect := widget.NewSelect(formatNames, nil)
targetFormatSelect.PlaceHolder = t("romConverter.targetFormat")
targetFormatSelect.Selected = string(romconverter.FormatFDS) // Default
...
sourceFormat := romconverter.FCFormat(sourceFormatSelect.Selected)
targetFormat := romconverter.FCFormat(targetFormatSelect.Selected)

// Create converter with progress callback
converter := romconverter.NewFCConverter(
selectedTool,
sourceEntry.Text,
targetEntry.Text,
sourceFormat,
