...
func TestGetFCFormats(t *testing.T) {
formats := GetFCFormats()
if len(formats) != 4 {
t.Errorf("expected 4 formats, got %d", len(formats))
}
expectedFormats := []FCFormat{FormatNES, FormatFDS, FormatUNF, FormatNSF}
for i, format := range formats {
if format != expectedFormats[i] {
t.Errorf("format[%d]: expected %s, got %s", i, expectedFormats[i], format)
}
...
func TestGetFCTools(t *testing.T) {
tools := GetFCTools()
if len(tools) < 2 {
t.Errorf("expected at least 2 tools, got %d", len(tools))
}
...
func TestFCConverterValidate(t *testing.T) {
converter := NewFCConverter(
ConversionTool{ID: "test", Name: "Test Tool"},
"",
"",
FormatNES,
